...
n_test=5001
truth_fun_vec = np.load('DATA/m_data.npy')[n_test,:]
...
d = np.load('DATA/noise_data_before_reshape_0.05.npy')[n_test,:]
d_clean = np.load('DATA/clean_data_before_reshape.npy')[n_test,:]
...
for j in range(pi):
    ...
    np.save('pCN/'+str(n_test)+'pcn'+str(j)+'.npy',samples_5000)
    print(accept / p_num)
