...
class GaussianElliptic2(object):
    ...
    def __init__(self, domain, alpha=1.0, a_fun=fe.Constant(1.0), theta=1.0, 
        ...
        self.eig_vec = np.load('RESULT/eig_vec_L2_' + str(self.n_eqx) + '.npy')
        eig_val=np.zeros(20)
        for i in range(20):
            eig_val[i]=1/(a_fun+self._alpha*(i*np.pi)**2)
    ...
    def generate_sample(self, method='numpy'):
        #eig_val = np.load('RESULT/eig_vals.npy')
