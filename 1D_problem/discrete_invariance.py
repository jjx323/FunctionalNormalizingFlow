...
for equ_nx in (50,75,100,200,300):
    dim=equ_nx+1
    k=1
    ...
    # torch.save(flow.state_dict(),'DATA')
    # newflow=NormalizingFlow(flow_length, data_dim)
    flow.load_state_dict(torch.load(path+model_load))
    #flow.load_state_dict(torch.load('TEST_16_noinitial_0.01'))
    num=1000
    ...
    for i in range(num):
        ...
        c=c+c_x_y
    c=c/num
    print('discreate level=',dim-1,'c(x,y)=',c,'where x=',x,'y=',y)
    ...
    print('discreate level=',dim-1,'L2_error=',L2)
    print('##################')
