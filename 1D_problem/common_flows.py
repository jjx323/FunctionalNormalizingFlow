...
class fourierFlow(nn.Module):
    def __init__(self):
        super().__init__()
        q=0.1
        self.R = nn.Parameter(q*torch.rand(m,m ,dtype=torch.cfloat))
        self.dia_=nn.Parameter(q*torch.rand(m))
        self.b = nn.Parameter(q*torch.rand(m,1, dtype=torch.cfloat))
        self.h=nn.Tanh()
        self.name='project_transformFlow'
    def constrained_r(self):
        r_=torch.triu(self.R, diagonal=1)
    ...
    def forward(self, z):
        ...
        z = z.to(torch.float32)
        size=z.shape[0]
        a=torch.fft.fft(z.T,dim=0)
        ft=a[:20,:]
        hidden_units = ft + self.b
        s=r@hidden_units
        rft=torch.zeros(101,size, dtype=torch.cfloat)
        rft[:20,:]=s
        x = z + (torch.fft.ifft(rft,dim=0)).T
        diag1=diag+1
        log_det = torch.log(torch.prod(diag1))
        return x, log_det
